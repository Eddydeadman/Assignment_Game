// GameCode.cpp	
// Created by Chris Rook
// Modified by 15010974
...
ErrorType Game::Update()
{
   ...
   // *********************************************************************
	...
	// update all components - if health = 0 -> end of game
	om.UpdateAll(gt.mdFrameTime);
	om.UpdateMovement(gt.mdFrameTime);	
	om.CheckCollisions();
	om.DeleteInactive();
	om.RenderAll();
...
// called when the game ends by returning to main menu
{
   ...
   // *********************************************************************
	...
	//delete all objects in the object manager
	om.DeleteAll();
